...
> 앱 실행 중 1분마다 현재 사용시간 저장하지말고 20분 단위로 하는게 나을거같다

- `_refresh_display()` tick 카운터를 `% 60` (1분)에서 `% 1200` (20분)으로 변경

> 커피마시고싶다. 차마시고싶다에 테두리 각각 넣어줘 분리해서  
> 그리고 버튼 눌렸을때 이벤트 같은게 있어야할거같은데

- 커피 버튼 `_ACCENT`(파랑), 차 버튼 `_GREEN`(초록) 개별 2px 픽셀 테두리
- 클릭 피드백: 테두리 색상을 `_YELLOW`로 700ms 플래시 + 피드백 레이블 `"커피 +1"` / `"차 +1"` 표시
...
- Mona12 폰트가 DirectWrite 경로에서 ColorEmoji 글리프 폴백을 지원하지 않아 이모티콘 렌더링 불가 확인
- `main_window.py` 전체 이모티콘 → 한국어 텍스트 치환 (15개 항목)
- `notification.py` 타이틀 바 `"🕹️  양방구는 게임중"` → `"양방구는 게임중"`

> 알람 토글버튼 생성해서 사용할지 안할지 여부도 넣어야겠다.  
> (아니 알람전체 on/off가 아니고 관리자 페이지에 알람목록에서 각 알람마다 토글 버튼 추가하고싶은거야)

- 처음에 전역 on/off로 구현 → 사용자 의도(개별 토글)로 수정
- `_build_fixed_tab()` / `_build_interval_tab()` 버튼 행에 `[ON/OFF]` 버튼 추가
- `_toggle_fixed()` / `_toggle_interval()` 메서드: 선택 알람의 `enabled` 반전 → 저장 → 목록 갱신 → 선택 유지

> 스프린트 11인거로 진행하자 이건

- Sprint 11 문서 생성: PLANNER / DESIGNER / DEVELOPER / QC / RESULT
- `docs/sprint-11/screenshots/capture.py` 생성
