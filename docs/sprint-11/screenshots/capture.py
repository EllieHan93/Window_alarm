"""Sprint 11 스크린샷 자동 캡처 (PIN 우회, 직접 모드 전환)."""
import sys
import time
...
# 샘플 알람 추가 (없으면)
if not config.get_fixed_alarms():
    config.upsert_fixed_alarm(FixedAlarmConfig(
        id=ConfigManager.new_id(), enabled=True,
        label="업무 시작", hour=9, minute=0, days=[], sound="default",
    ))
    config.upsert_fixed_alarm(FixedAlarmConfig(
        id=ConfigManager.new_id(), enabled=False,
        label="점심 알림", hour=12, minute=0, days=[0,1,2,3,4], sound="beep",
    ))
if not config.get_interval_alarms():
    config.upsert_interval_alarm(IntervalAlarmConfig(
        id=ConfigManager.new_id(), enabled=True,
        label="휴식 알림", interval_minutes=60, sound="beep",
    ))
...
def step3_mgmt():
    ...
    root.update_idletasks()
    root.after(400, step4_snap_fixed)


def step4_snap_fixed():
